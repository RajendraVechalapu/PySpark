...
def main():
    ...
    latest_video_file = max(video_files, key=lambda x: os.path.getctime(os.path.join(directory, x)))
    print("File Modifying: "+ latest_video_file)

    file_in = os.path.join(directory, latest_video_file)
    file_out = os.path.join(directory, "edited_" + latest_video_file)
    ...
    vid.close()
     ...
     # Open the edited file with the default application
    subprocess.run(["start", "", file_out], shell=True)
